	...
	try{
		...
		String totalSql = "SELECT count(*) as cnt "
		 		+ "  FROM board b      "
		   		+ " INNER JOIN member m"
		   		+ " ON b.mno = m.mno   "
				+ " WHERE b.delyn = 'N' AND b.type = 'K'";  // K 게시판만 선택!
if(searchType != null){
...
pagingVO = new PagingVO(nowPage,totalCnt,10);
		...
		String sql = "SELECT bno, btitle, m.mnickname, b.rdate "
		 		+ " FROM board b "
		   		+ " INNER JOIN member m "
		   		+ " ON b.mno = m.mno   "	
				+ " WHERE b.delyn = 'N' AND b.type = 'K'"; //지식 게시글만 가져옴!
  ...
  <section class="about_section layout_padding">
    <div class="container  ">
      <div class="heading_container heading_center">
        ...
        <p>
          지식을 공유하고 새로운 아이디어를 얻어보세요!
        </p><br>

		<form name="frm" action="kboard.jsp" method="get">
          ...
          <tbody>
			...
			while(rs.next()){
				...
				%>
					<tr>
						<td><%=bno%></td>
						<td><a href="kview.jsp?bno=<%=bno%>" style="color: pink;"><%=btitle%></a></td>
						<td><%=mnickname%></td>
						<td><%=rdate%></td>
					</tr>
	...
	location.href='login.jsp';
		...
		<!-- 페이징 영역 -->
			<div class="paging">
		 	...
		 	%>
		 		<a href="kboard.jsp?nowPage=<%=pagingVO.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">이전</a>
		 	<%		
		 		}
 				...
 				<%
		 				...
		 				if(searchType != null){
				%>
			 	<a href="kboard.jsp?nowPage=<%=i%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%=i %></a>
			 	<%		
		 				}else{
 				%>
 			 	<a href="kboard.jsp?nowPage=<%=i%>"><%=i %></a>
 			 	<%	
		 				}
		 			}
		 		}
		 		...
		 		if(pagingVO.getEndPage() < pagingVO.getLastPage()){
		 			%>
		 		<a href="kboard.jsp?nowPage=<%=pagingVO.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">다음</a>
		 			<%
		 		}
		 	%>
		 </div>
