...
<%
    ...
    if(member != null){
        ...
        try{
            ...
            // 부모 댓글의 기본키인 rno를 가져오기 위한 SQL 쿼리
            String getParentRnoSql = "SELECT rno FROM reply WHERE rno = ?";
            psmt = conn.prepareStatement(getParentRnoSql);
            psmt.setInt(1, reply.getRno()); // 부모 댓글의 rno를 설정
            ResultSet parentRnoRs = psmt.executeQuery();
            
            if(parentRnoRs.next()) {
                // 부모 댓글의 rno가 존재하면 parent_rno에 설정
                reply.setParent_rno(parentRnoRs.getInt("rno"));
                ...
                psmt.setInt(4, reply.getParent_rno());
