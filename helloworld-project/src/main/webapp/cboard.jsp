	...
	try{
		...
		String totalSql = "SELECT count(*) as cnt "
		 		+ "  FROM board b      "
		   		+ " INNER JOIN member m"
		   		+ " ON b.mno = m.mno   "
				+ " WHERE b.delyn = 'N' AND b.type = 'C'";  //커뮤니티 게시글만 가져옴!
if(searchType != null){
  ...
  <section class="about_section layout_padding">
    <div class="container  ">
      <div class="heading_container heading_center">
        ...
        <p>
          다양한 사람들을 만나고 생각의 폭을 넓혀보세요.
        </p><br>

		<form name="frm" action="cboard.jsp" method="get">
			...
			<!-- 검색값(searchValue)이 null일 때 빈 문자열을 출력하도록 처리한 것입니다. 현재 코드에서는 검색값이 null이면 out.print(""); 
				부분이 실행되어 빈 문자열이 해당 input 태그의 value 속성에 대입됩니다. 
				검색값이 null이 아닌 경우에는 out.print(""); 부분이 생략되어 검색값이 해당 input 태그의 value 속성에 대입됩니다. -->
          ...
          <tbody>
			...
			while(rs.next()){
				...
				%>
					<tr>
						<td><%=bno%></td>
						<td><a href="cview.jsp?bno=<%=bno%>" style="color: pink;"><%=btitle%></a></td>
						<td><%=mnickname%></td>
						<td><%=rdate%></td>
					</tr>
	...
	location.href='login.jsp';
		...
		<!-- 페이징 영역 -->
			<div class="paging">
		 	...
		 	%>
		 		<a href="cboard.jsp?nowPage=<%=pagingVO.getStartPage()-1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">이전</a>
		 	<%		
		 		}
 				...
 				<%
		 				...
		 				if(searchType != null){
				%>
			 	<a href="cboard.jsp?nowPage=<%=i%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>"><%=i %></a>
			 	<%		
		 				}else{
 				%>
 			 	<a href="cboard.jsp?nowPage=<%=i%>"><%=i %></a>
 			 	<%	
		 				}
		 			}
		 		}
		 		...
		 		if(pagingVO.getEndPage() < pagingVO.getLastPage()){
		 			%>
		 		<a href="cboard.jsp?nowPage=<%=pagingVO.getEndPage()+1%>&searchType=<%=searchType%>&searchValue=<%=searchValue%>">다음</a>
		 			<%
		 		}
		 	%>
		 </div>
