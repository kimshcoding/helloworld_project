...
try {
	...
	// 이전글 bno 가져오기
		...
		sql = "SELECT * FROM board WHERE type = 'Q' and delyn='N' and bno < ? order by bno DESC limit 0,1 ";  // ? -> 현재 페이지 bno
	...
	// 다음글 bno 가져오기
		...
		sql = "SELECT * FROM board WHERE type = 'Q' and delyn='N' and bno > ? order by bno ASC limit 0,1 ";  // ? -> 현재 페이지 bno
...
<!-- 이전글, 다음글, 목록 버큰 위치 ---------------------------------------------------------------------------- -->	
	...
	<div class="d-grid gap-3 d-md-flex justify-content-md-end">
		...
		%> --%>
		<button onclick="location.href='qview.jsp?bno=<%= before %>'" class="btn btn-primary me-md-3" type="button">이전글</button>
		<%
			}
		%>
		...
		<button onclick="location.href='qview.jsp?bno=<%= after %>'" class="btn btn-primary me-md-3" type="button">다음글</button>
		<%
			}
		%>
		
		<!-- 클릭시 게시글 목록 페이지로 이동 -->
		<button onclick="location.href='qboard.jsp'" class="btn btn-success">목록</button>
	</div>
		<br>		
...
<!-- -----------------------------------------------------------------------------------------------------------	 -->			
			...
			<div class="heading_container heading_center">
				...
				<div class="d-grid gap-2 d-md-block">
					<button
						onclick="location.href='qmodify.jsp?bno=<%=board.getBno()%>'"
			...
			function delFn(){
				...
				<form name="frm" action="qdelete.jsp" method="post">
				...
				<div class="d-flex justify-content-center align-items-center">
					<form name="replyfrm">
						...
						댓글 <input type="text" name="rcontent" maxlength="100"
		 ...
		 <!-- 댓글 100글자수 이상 입력 불가능 -->
				<script>
				// 댓글 입력 길이를 확인하는 함수 checkInputLength
		 		function checkInputLength(inputElement) {
		 		// 입력된 댓글의 길이가 100글자를 초과하는지 확인
		        if (inputElement.value.length > 100) {
		        	// 만약 100글자를 초과하면 사용자에게 알림을 표시
		            alert("댓글은 100글자 이하로 입력해주세요.");
		            // 댓글을 100글자로 자르고 다시 입력란에 설정
