...
try {
	...
	// 이전글 bno 가져오기
		...
		sql = "SELECT * FROM board WHERE type = 'C' and delyn='N' and bno < ? order by bno DESC limit 0,1 ";  // ? -> 현재 페이지 bno
	...
	// 다음글 bno 가져오기
		...
		sql = "SELECT * FROM board WHERE type = 'C' and delyn='N' and bno > ? order by bno ASC limit 0,1 ";  // ? -> 현재 페이지 bno
...
<!-- 이전글, 다음글, 목록 버큰 위치 ---------------------------------------------------------------------------- -->	
	...
	<div class="d-grid gap-3 d-md-flex justify-content-md-end">
		...
		%> --%>
		<button onclick="location.href='cview.jsp?bno=<%= before %>'" class="btn btn-primary me-md-3" type="button">이전글</button>
		<%
			}
		%>
		...
		<button onclick="location.href='cview.jsp?bno=<%= after %>'" class="btn btn-primary me-md-3" type="button">다음글</button>
		<%
			}
		%>
		
		<!-- 클릭시 게시글 목록 페이지로 이동 -->
		<button onclick="location.href='cboard.jsp'" class="btn btn-success">목록</button>
	</div>
		<br>		
...
<!-- -----------------------------------------------------------------------------------------------------------	 -->			
			...
			<div class="heading_container heading_center">
				...
				<div class="d-grid gap-2 d-md-block">
					<button
						onclick="location.href='cmodify.jsp?bno=<%=board.getBno()%>'"
			...
			function delFn(){
				...
				<form name="frm" action="cdelete.jsp" method="post">
